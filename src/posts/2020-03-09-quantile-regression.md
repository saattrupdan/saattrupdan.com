...
Let's start with a simple linear example, where we sample $n=1000$ training data points $X\sim\textsf{Unif}(0,5)$ and define our response variable as $Y = 5X - 5 + \varepsilon$ with $\varepsilon\sim N(0,1)$. For the testing data I've chosen $100$ equidistributed points in the interval $[0,5]$. Implementing linear quantile regression is simple using the [statsmodels](https://www.statsmodels.org/stable/index.html) module:
...
![Plot of quantile linear regression prediction interval, where the interval encloses most of the residuals](/quantile-linear-regression.png)
...
![Residual- and prediction interval plot for the overfitting MLP. The residuals are much larger in magnitude, and the intervals are incredibly narrow around each residual.](/quantile-non-linear-overfit.png)
