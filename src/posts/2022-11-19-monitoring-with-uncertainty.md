...
![Table showing that our Doubt method has better coverage than the competing methods from NASA and MAPIE.](/src/assets/doubt-coverage-results.webp)
...
![Plot showing the correlation of the model's mean absolute error with the uncertainty measure from Doubt.](/src/assets/uncertainty-correlates-with-mae.webp)
...
![Table showing that our Doubt method estimates the model's prediction errors better than the competing methods from NASA and MAPIE.](/src/assets/doubt-monitoring-results.webp)
...
![A SHAP plot showing that all three methods locate the first two shifted features, but PSI and K-S also attributes a substantial part of the deterioration to the random feature, whereas our SHAP method does not.](/src/assets/uncertainty-shap.webp)
