...
but which can be applied to any graph structure to get an idea of the most *important*
...
> **Definition**. For a *state space* $S\subseteq\mathbb N_0$ we define a **Markov
...
that we have a graph of interest $G$, and we would like to find the *most central*
...
We start by making the graph *complete*, meaning that we add edges between every pair
of nodes. Let's call this graph the *completion* of $G$, denoted by $\overline G$.
...
Every old edge is going to get the transition probability $\tfrac{\alpha}{n_\text{in}}
...
instead of creating new links between *all* node pairs, we start with a specified set
$S$ of nodes, and only add edges from all nodes *into* nodes from $S$.
