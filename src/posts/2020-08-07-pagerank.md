...
![Diagram of a random web surfer](/random-web-surfer.webp)
...
![A graph containing seven nodes, with various edges between them, usually going from a lower node to a higher one. The edges are labelled with numbers between 0 and 1, with the numbers on the outgoing edges of a node sum to one](/markov-chain.webp)
...
![A graph containing four nodes, which is completed by drawing new edges between every pair of nodes that did not already have an edge](/graph-completion.webp)
